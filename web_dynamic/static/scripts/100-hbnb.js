// Script that implements filtering by State and City, updating locations on checkbox change
'use strict';
$(() => {
  ...
  $(selectors.stateBox).change(ev => {
    ...
    const statePred = obj => (
      (obj.id === stateId) && (obj.type === LocationList.state)
    );
    ...
    const htmlContent = selectedLocations.map(obj => obj.name).join(', ');
  ...
  $(selectors.cityBox).change(ev => {
    ...
    if (ev.target.checked) {
      ...
      if (stateCheckBox.checked) {
        ...
        selectedLocations = selectedLocations.filter(obj => !cityPred(obj));
      }
    }
    const htmlContent = selectedLocations.map(obj => obj.name).join(', ');
    $(selectors.locationsHeader).html(
  ...
  $('section.filters > button').click(() => {
    const filter = {
      states: selectedLocations
        .filter(obj => obj.type === LocationList.state)
        .map(obj => obj.id),
      cities: selectedLocations
        .filter(obj => obj.type === LocationList.city)
        .map(obj => obj.id),
